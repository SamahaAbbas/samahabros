...
class Admins extends BaseModel
{
    ...
    public function saveUsers($pdata, $editId = null)
    {
        ...
        if (!$editId) {
            ...
            $userInfo = $this->getInfo($editId);
            if ($userInfo) {
                $currentUname =  $userInfo->username;
                if ($currentUname != $pdata["username"]) {

                    $newUname = $pdata["username"];
                    $columnsArr["username"] = $newUname;

                    //update all users creators for current admin
                    \App\Models\Users::where("admin_uname", $currentUname)->update(["admin_uname" => $newUname]);
                }
            }
        }
